...
```{r}
str <- str_glue("\\comment[id=LRN]{{Generated using R. Don't modify here.}}First, we rank the configurations with respect to the average cpu time for all solved instances. The sequence from best to worst becomes {knitr::combine_words(res)}, where the increase in percentages compared to the best configuration is given in parentheses.")
...
dat <- datResultSolvedI %>% #filter(instance_name == "Kirlik14-ILP_p-3_n-30_m-15_ins-3") %>%
  ...
  summarize(cpu = mean(nbNodes)) %>% print
  
  
  
  mutate(win_bound = case_when(
  ...
  group_by(win_bound, class) %>%
  summarize(ctr = n(), diff = mean(diff), pct = mean(pct), cpuTotal = mean(cpuTotal)) %>% 
  mutate(ctr_pct = round(100 * ctr/sum(ctr))) %>% 
  group_by(class) %>% 
  arrange(desc(ctr_pct), .by_group = T) %>% print()
str <- str_glue(
  "\\comment[id=LRN]{{Generated using R. Don't modify here.}}We are here interested in determining whether one rule for finding the bound (\\MED, \\MOFV and \\RANS) is consistently better than the other when considering non-binary integer problems. Since the effect on the branching tree of using \\LP vs \\WLP is negligible, we will consider the \\WLP configuration here. By considering the performance profiles in \\autoref{{fig:performanceProfiles}} we see that there is no clear winner among {knitr::combine_words(dat$win_bound)}. 
  
The {dat$win_bound[1]}-configuration performed best in {dat$ctr_pct[1]}\\% of the instances and on average the cpu time difference was {round(mean(dat$diff))} seconds (a {round(100*mean(dat$pct))}\\% increase compared to the lowest cpu time)",
...
dat <- datResultSolvedI %>% 
  ...
  group_by(instance_name) %>%
  pivot_wider(names_from = configLB, values_from = c(cpuTotal, nbLpSolved, nbNodes)) %>% 
  ungroup() %>%
  group_by(class, p) %>%
...
dat <- datLBStat %>% 
  ...
  summarise(across(starts_with("avg"), mean)) %>% print
  
dat %>%   
  pivot_longer(cols = starts_with("avg")) %>% 
  ggplot(aes(x = depth, y = value, color = name)) +
  geom_line() +
  facet_grid(rows = vars(class, configLB), cols = vars(p), scales = "free")
