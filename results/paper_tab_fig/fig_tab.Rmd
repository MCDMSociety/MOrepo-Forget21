...
In a few instances \\LP performed best (fastest in {res4}\\% of the instances). We will have a closer look at the reason for this in \\autoref{{sec:details}}",
...
p <- ggplot(aes(x = n, y = nbNodes, color = configValSplit), data = dat) +
  geom_point(size = 0.5) +
  geom_smooth(se = F, size = 0.5) +
  geom_smooth(se = F, size = 0.5, method = "lm", data = dat %>% filter(class == "ILP", p == "$p = 5$")) +
...
p1 <- dat %>%  
  ...
  scale_color_manual(values = wes_palette(24, name = "Royal1", type = "continuous")) +
...
dat <- datResultSolvedI %>% 
  ...
  mutate(fct = cpuTotal_WLP/cpuTotal_WLPUB, 
         ...
         pctNodesDesc = 100*(nbNodes_WLP-nbNodes_WLPUB)/nbNodes_WLP
        ) %>% 
  group_by(p, class) %>% 
  summarise(across(where(is.double), mean)) 
...
```{r, eval=FALSE}
# testing eps-approx defs

# I am not sure how you normalize e.g. are you assuming the same nadir and ideal point for both the ub and nd set?
# e.g. if 
y <- matrix(c(1,4, 3,2), byrow = T, ncol = 2)
u <- y + 1
plot(y, col = "blue", xlim=c(0,6), ylim=c(0,6))
points(u, pch = 2, col = "red")
# then 
iY <- c(min(y[,1]), min(y[,2]))
iY
dY <- c(max(y[,1]), max(y[,2])) - iY
dY
# iU <- c(min(u[,1]), min(u[,2]))
# dU <- c(max(u[,1]), max(u[,2])) - iU
Norm <- function(p, d, i, rng = 100) {
  return(rng *(p-i)/d)
}
t(apply(u, 1, Norm, d = dY, i = iY))
t(apply(y, 1, Norm, d = dY, i = iY))
...
epsY <- function(y, uSet) {
  min(apply(uSet, 1, function(x) epsUY(x,y)))
}
eps <- function(y, u) {
  iY <- c(min(y[,1]), min(y[,2]))
  dY <- c(max(y[,1]), max(y[,2])) - iY
  uB <- t(apply(u, 1, Norm, d = dY, i = iY))
  yB <- t(apply(y, 1, Norm, d = dY, i = iY))
  e <- apply(yB, 1, function(x) epsY(x, uB))
  return(list(eps = max(e), epsAvg = mean(e)))
} 

## Using the other approach $\bar{y_i} -\epsilon \Delta_i \leqq y_i, \Delta_i = y^N_i-y^I_i$
epsUYD <- function(u, y, d) {
  max((u-y)/d)
}
epsYD <- function(y, uSet, dY) {
  min(apply(uSet, 1, function(x) epsUYD(x, y, dY)))
}
epsD <- function(y, u) {
  iY <- c(min(y[,1]), min(y[,2]))
  dY <- c(max(y[,1]), max(y[,2])) - iY
  e <- apply(y, 1, function(x) epsYD(x, u, dY))
  return(list(eps = max(e), epsAvg = mean(e)))
} 


## Testing the different approaches
y <- matrix(c(1,4, 3,2), byrow = T, ncol = 2)
u <- y + 1
eps(y, u)
epsD(y, u)

u <- matrix(c(2,6, 3,3, 6,2), byrow = T, ncol = 2)
y <- matrix(c(1,4, 3,2), byrow = T, ncol = 2)
plot(y, col = "blue", xlim=c(0,6), ylim=c(0,6))
points(u, pch = 2, col = "red")
eps(y, u)
...
u <- matrix(c(2,6, 3,3, 6,2), byrow = T, ncol = 2)
y <- matrix(c(2,6, 3,3, 6,2), byrow = T, ncol = 2)
plot(y, col = "blue", xlim=c(0,6), ylim=c(0,6))
points(u, pch = 2, col = "red")
eps(y, u)
epsD(y, u)

u <- matrix(c(2,6, 4,4, 6,2), byrow = T, ncol = 2)
y <- matrix(c(2,6, 3,3, 6,2), byrow = T, ncol = 2)
plot(y, col = "blue", xlim=c(0,6), ylim=c(0,6))
points(u, pch = 2, col = "red")
eps(y, u)
...
u <- matrix(c(2,6, 4,4, 6,2)+100, byrow = T, ncol = 2)
y <- matrix(c(2,6, 3,3, 6,2), byrow = T, ncol = 2)
plot(rbind(y,u), type="n")
points(u, col = "blue")
points(y, pch = 2, col = "red")
eps(y, u)
epsD(y, u)

u <- matrix(c(2,6, 4,4, 6,2)+1000, byrow = T, ncol = 2)
y <- matrix(c(2,6, 3,3, 6,2), byrow = T, ncol = 2)
plot(rbind(y,u), type="n")
points(u, col = "blue")
points(y, pch = 2, col = "red")
...
u <- matrix(c(2,12, 5,5, 6,2), byrow = T, ncol = 2)
y <- matrix(c(2,6, 3,3, 6,2), byrow = T, ncol = 2)
plot(rbind(y,u), type="n")
points(u, col = "blue")
points(y, pch = 2, col = "red")
