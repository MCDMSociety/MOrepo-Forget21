...
```{r color scale palette used, include=FALSE, eval=FALSE}
...
show_col(mypal)
mypal <- brewer.pal(n = 11, name = "RdYlBu")
show_col(mypal)
...
datUBStart <- read_csv(here("results/convert/csv/resultsUbWarmstart.csv")) %>% 
  mutate(
    ...
    alg_config = str_c("\\", configLB, "-\\", toupper(configValSplit), "-UB")) %>% view
...
p <- ggplot(aes(x = n, y = nd), data = dat) +
  geom_point(aes(alpha = solved, color = factor(p)), size = 0.75) + 
...
```{r}
p <- ggplot(datResult, aes(x = n, y = cpuTotal, color = configValSplit, linetype = configLB)) +
  geom_point(size = 0.5) +
...
if (copyTabFigs) {
  tikz(file = here(pathOverLeaf, "fig_cpu.tex"), standAlone=F, width = 7, height = 6)
  print(p)
  dev.off()
}
```
...
if (copyTabFigs) {
  tikz(file = here(pathOverLeaf, "fig_oss.tex"), standAlone=F, width = 7, height = 6)
  print(p)
  dev.off()
}
```
