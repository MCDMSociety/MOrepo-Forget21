...
Groups with unsolved instances and cpu:
...
datResult %>% 
  filter(solved == 1, configLB == "WLP") %>% 
    mutate(pct = mean(lastNDCpu/cpuTotal)) %>%  view() %>% 
...
dat <- datResult %>% 
  ...
  group_by(instance_name) %>% 
  filter(n() == 2) %>% 
  mutate(alg_config = str_remove_all(alg_config, "\\\\")) %>% print() 
...
if (copyTabFigs) {
  tikz(file = here(pathOverLeaf, "fig_leaf.tex"), standAlone=F, width = 7, height = 6)
  print(p)
  dev.off()
}
```
